  ...
  if(EXISTS "${_c}")
    ...
    if(_ldcandidates)
      ...
      if(_found_musl_lib EQUAL -1)
        list(APPEND CMAKE_EXE_LINKER_FLAGS "${_c_dir_flags}")
      endif()
      ...
      list(FIND CMAKE_SHARED_LINKER_FLAGS_INIT "${_c_dir_flags}" _found_musl_lib2)
      if(_found_musl_lib2 EQUAL -1)
        list(APPEND CMAKE_SHARED_LINKER_FLAGS_INIT "${_c_dir_flags}")
      endif()
...
if(CMAKE_PLATFORM_SUPPORTS_SHARED_LIBS AND _GENERIC_MUSL_HAVE_CLANG)
  ...
  if(LD_LLD OR LLD_LINK OR LLVM_LLD)
    ...
    foreach(type SHARED MODULE EXE)
      if (NOT DEFINED CMAKE_${type}_LINKER_FLAGS_INIT)
  ...
  if(_musl_loader)
    ...
    if(_found_lld_veneer_flag EQUAL -1)
      list(APPEND CMAKE_EXE_LINKER_FLAGS "-Xlinker --pic-veneer")
    endif()
    list(FIND CMAKE_EXE_LINKER_FLAGS "--unique" _found_uniq_flag)
    if(_found_uniq_flag EQUAL -1)
      list(APPEND CMAKE_EXE_LINKER_FLAGS "-Xlinker --no-gnu-unique")
      list(APPEND CMAKE_EXE_LINKER_FLAGS "-Xlinker --unique")
    endif()
    list(FIND CMAKE_EXE_LINKER_FLAGS "--dynamic-linker" _found_dyn_link_flag)
    if(_found_dyn_link_flag EQUAL -1)
      list(APPEND CMAKE_EXE_LINKER_FLAGS "-Xlinker --dynamic-linker=${_musl_loader}")
    endif()
    list(FIND CMAKE_EXE_LINKER_FLAGS "--pack-dyn-relocs=relr" _found_dyn_reloc_flag)
    if(_found_dyn_reloc_flag EQUAL -1)
      list(APPEND CMAKE_EXE_LINKER_FLAGS "-Xlinker --pack-dyn-relocs=relr")
    endif()
    list(FIND CMAKE_SHARED_LINKER_FLAGS_INIT "--pic-veneer" _found_lld_veneer_flag2)
    if(_found_lld_veneer_flag2 EQUAL -1)
      list(APPEND CMAKE_SHARED_LINKER_FLAGS_INIT "-Xlinker --pic-veneer")
    endif()
    list(FIND CMAKE_SHARED_LINKER_FLAGS_INIT "--unique" _found_uniq_flag2)
    if(_found_uniq_flag2 EQUAL -1)
      list(APPEND CMAKE_SHARED_LINKER_FLAGS_INIT "-Xlinker --no-gnu-unique")
      list(APPEND CMAKE_SHARED_LINKER_FLAGS_INIT "-Xlinker --unique")
    endif()
    list(FIND CMAKE_SHARED_LINKER_FLAGS_INIT "--dynamic-linker" _found_dyn_link_flag2)
    if(_found_dyn_link_flag2 EQUAL -1)
      list(APPEND CMAKE_SHARED_LINKER_FLAGS_INIT "-Xlinker --dynamic-linker=${_musl_loader}")
    endif()
    list(FIND CMAKE_SHARED_LINKER_FLAGS_INIT "--pack-dyn-relocs=relr" _found_dyn_reloc_flag2)
    if(_found_dyn_reloc_flag2 EQUAL -1)
      list(APPEND CMAKE_SHARED_LINKER_FLAGS_INIT "-Xlinker --pack-dyn-relocs=relr")
    endif()
  ...
  list(FIND CMAKE_EXE_LINKER_FLAGS "relro" _found_relro_flag)
  if(_found_relro_flag EQUAL -1)
    list(APPEND CMAKE_EXE_LINKER_FLAGS "-z relro")
    list(APPEND CMAKE_EXE_LINKER_FLAGS "-z now")
  endif()
  if (NOT DEFINED CMAKE_SHARED_LINKER_FLAGS)
    set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS_INIT}")
  endif()
  list(FIND CMAKE_SHARED_LINKER_FLAGS "relro" _found_relro_flag2)
  if(_found_relro_flag2 EQUAL -1)
    list(APPEND CMAKE_SHARED_LINKER_FLAGS "-z relro")
    list(APPEND CMAKE_SHARED_LINKER_FLAGS "-z now")
  endif()
...
if(DEFINED CLANG_RT_PATH AND EXISTS "${CLANG_RT_PATH}")
  ...
  if(_found_clang_rt EQUAL -1)
    list(APPEND CMAKE_EXE_LINKER_FLAGS "${CLANG_RT_PATH}")
    list(APPEND CMAKE_EXE_LINKER_FLAGS "-Xlinker --exclude-libs=libgcc_s.so.1")
    list(APPEND CMAKE_EXE_LINKER_FLAGS "-Xlinker --exclude-libs=libgcc_s.so")
    list(APPEND CMAKE_EXE_LINKER_FLAGS "-Xlinker --exclude-libs=libgcc_s.a")
  endif()
  list(FIND CMAKE_SHARED_LINKER_FLAGS "${CLANG_RT_PATH}" _found_clang_rt2)
  if(_found_clang_rt2 EQUAL -1)
    list(APPEND CMAKE_SHARED_LINKER_FLAGS "${CLANG_RT_PATH}")
    list(APPEND CMAKE_SHARED_LINKER_FLAGS "-Xlinker --exclude-libs=libgcc_s.so.1")
    list(APPEND CMAKE_SHARED_LINKER_FLAGS "-Xlinker --exclude-libs=libgcc_s.so")
    list(APPEND CMAKE_SHARED_LINKER_FLAGS "-Xlinker --exclude-libs=libgcc_s.a")
  endif()
endif()
