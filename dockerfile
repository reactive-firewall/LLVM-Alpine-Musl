...
RUN set -eux; \
    ...
    ${SYSROOT:-/sysroot}${MUSL_PREFIX:-/usr}/bin/${TARGET_TRIPLE}-c++ -v -fsyntax-only -std=c++17 -nostdlibinc -nostdinc++ -cxx-isystem /headers/usr/include/c++/v1 -isystem /headers/usr/include /tmp/test.cpp ;
