...
RUN ./configure --prefix=${MUSL_PREFIX} --target=${TARGET_TRIPLE} \
      ...
      LDFLAGS="${LDFLAGS}" \
      LIBCC="${SYSROOT}/lib/linux/${LLVM_RTLIB}" \
      CFLAGS="${CFLAGS} --sysroot=$SYSROOT -rtlib=compiler-rt -fno-math-errno -fPIC -fno-common -fuse-ld=ld.lld" && \
    make -j"$(nproc)" && \
...
ENV LDFLAGS="-Wl,--sysroot=/sysroot -Wl,--dynamic-linker=/lib/libc.so -fuse-ld=lld"
ENV CFLAGS="-rtlib=compiler-rt -fPIC"
ENV CXXFLAGS="-stdlib=libc++ -rtlib=compiler-rt -fPIC"
...
RUN cmake -S runtimes -B build-runtimes -G "Ninja" \
