...
# Musl can expose BSD-compatible vasprintf, may need to expose those for use by libcxx. (depends on BSD)
...
RUN ./configure --prefix=${MUSL_PREFIX} --target=${TARGET_TRIPLE} \
      ...
      AR=llvm-ar RANLIB=llvm-ranlib \
      LDFLAGS="${LDFLAGS}" \
      LIBCC="-Wl,--whole-archive ${SYSROOT:-/sysroot}${MUSL_PREFIX:-/usr}/lib/generic/${LLVM_RTLIB} -Wl,--no-whole-archive" \
      CXXFLAGS="${CXXFLAGS}" \
...
# unfortunately we can't just redirect g++ to clang (cmake will FORCE g++ and fail)
#RUN set -eux && \
#    printf '%s\n' '#!/bin/sh' 'exec clang++ --no-default-config "$@"' >"/usr/bin/g++" && \
#    chmod +x "/usr/bin/g++" ;
...
RUN mkdir -p /bootstrap/libcxxrt && cd libcxxrt-project && \
  cmake --log-level=VERBOSE -S . -B ../libcxxrt -G Ninja \
    ...
    -DCMAKE_CXX_FLAGS="-stdlib=libc++ -std=c++11 ${CXXFLAGS} ${CFLAGS} -Qunused-arguments" \
    -DCMAKE_COLOR_DIAGNOSTICS=ON \
  ...
  if [ -r "libcxxrt-project/src/cxxabi.h" ] ; then \
    ...
    install -m 0644 libcxxrt-project/src/unwind.h "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/unwind-cxxabi.h" && \
    ...
    install -m 0644 libcxxrt-project/src/unwind-arm.h "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/unwind-arm.h" && \
    install -m 0644 libcxxrt-project/src/unwind-itanium.h "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/unwind-itanium.h" && \
    install -m 0644 libcxxrt-project/src/cxxabi.h "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/cxxabi.h" && \
    { touch -d "${SOME_DATE_EPOCH}" "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/unwind-cxxabi.h" || true ;} && \
    { touch -d "${SOME_DATE_EPOCH}" "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/__libunwind_config.h" || true ;} && \
    { touch -d "${SOME_DATE_EPOCH}" "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/unwind-llvm.h" || true ;} && \
    { touch -d "${SOME_DATE_EPOCH}" "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/unwind.h" || true ;} && \
    { touch -d "${SOME_DATE_EPOCH}" "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/unwind-arm.h" || true ;} && \
    { touch -d "${SOME_DATE_EPOCH}" "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/unwind-itanium.h" || true ;} && \
    { touch -d "${SOME_DATE_EPOCH}" "${SYSROOT:-/sysroot}/usr/include/c++/v1/cxxabi/cxxabi.h" || true ;} && \
...
ENV CFLAGS="--target=${TARGET_TRIPLE} -rtlib=compiler-rt -fPIC -Xlinker --pic-veneer -ffunction-sections -fdata-sections -D_ALL_SOURCE -DSANITIZER_CAN_USE_PREINIT_ARRAY=0 -isysroot ${SYSROOT:-/sysroot} -iwithsysroot /usr/include"
# might need -nostdinc++
...
# Does NOT require -D__ELF__
ENV CFLAGS="--no-default-config --target=${TARGET_TRIPLE} -rtlib=compiler-rt -fPIC -Xlinker --pic-veneer -ffunction-sections -fdata-sections -D_ALL_SOURCE -DSANITIZER_CAN_USE_PREINIT_ARRAY=0 -I${SYSROOT:-/sysroot}/usr/include"
# might need -nostdinc++
...
# may want linker flag -Xlinker --nostdlib to prevent linking to any std c++
# may want to link -Xlinker -l${LLVM_RTLIB_STUB}
ENV LDFLAGS="-fuse-ld=lld -v -Xlinker --sysroot=${SYSROOT:-/sysroot} -Xlinker -L -Xlinker ${SYS_LIB} -Xlinker -L -Xlinker ${SYSROOT:-/sysroot}/lib -Xlinker -L -Xlinker ${SYS_LIB}/generic -Xlinker -l${LLVM_RTLIB_STUB} -Xlinker --exclude-libs=libgcc_s.so.1 -Xlinker --exclude-libs=libgcc_s.so -Xlinker --dynamic-linker=${SYSROOT:-/sysroot}/lib/${MUSL_LDLIB}"
# Does NOT require -D__ELF__
ENV CFLAGS="--no-default-config --target=${TARGET_TRIPLE} -rtlib=compiler-rt -fPIC -Xlinker --pic-veneer -ffunction-sections -fdata-sections -D_ALL_SOURCE -D_POSIX_C_SOURCE=200809L -D_XOPEN_SOURCE=700 -DSANITIZER_CAN_USE_PREINIT_ARRAY=0 -I${SYSROOT:-/sysroot}/usr/include"
# might need -nostdinc++
ENV CXXFLAGS="-ffunction-sections -fdata-sections --unwindlib=${SYSROOT:-/sysroot}${MUSL_PREFIX:-/usr}/lib/libunwind.so.1.0"
